<?php
    ...
    class HelperCommand
    {
        ...
        public function __construct(
            readonly ManagerRegistry $doctrine,
            readonly DoctrineEncryptorService $doctrineEncryptorService,
            readonly ParameterBagInterface $parameterBag,
            readonly OpenSSLSymEncryptor $openSSLSymEncryptor,
            readonly HaliteEncryptor $haliteEncryptor
        ) {
        }
            ...
            foreach ($metadata as $classMetadata) {
                ...
                foreach ($properties as $property) {
                    ...
                    $neoxEncryptorAttributes = array_filter(
                        ...
                        fn($attribute) => $attribute->getName() === NeoxEncryptor::class
                    );
                    ...
                    if (!empty($neoxEncryptorAttributes)) {
                        ...
                        if ($firstAttribute !== false) {
                            ...
                            foreach ($neoxEncryptorAttributes as $attribute) {
                                ...
                                $propertiesList[]                  = $type ? sprintf(
                                    '   Encryptor : %s - Property : %s ( %s%s ) ',
                                    $buildIn,
                                    $property,
                                    $type,
                                    $length
        ...
        private function getCountEntity($entity): int
        {
            ...
            $count         = $repository->createQueryBuilder('e')->select(
                'COUNT(e.id)'
            ) // Supposons que 'id' est le nom de votre colonne ID
